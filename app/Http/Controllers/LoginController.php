    ...
    public function postlogin(Request $request)
{
    ...
    if (Auth::attempt($credentials)) {
        $user = Auth::user();
        if ($user->role === 'admin') {
            return redirect()->route('dashboard')->with('success', 't');
        }
        ...
        if ($pendaftar && $pendaftar->status == 'terima') {
            ...
            Auth::logout();
            return redirect()->route('login')->with('failed', 'Status pendaftar tidak diterima.');
        }
        else{
            Auth::logout();
            return redirect()->route('login')->with('failed', 'Status belum diterima.');
        }
    } else {
        return redirect()->route('login')->with('failed', 'Email atau Password Salah');
    }
